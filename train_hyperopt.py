...
parser.add_argument('--max_evals', type=int, default=50, help='Maximum number of hyperopt evaluation [default: 50]')
...
def hyperopt():
    ...
    start_time_hype = time.time()

    trials = Trials()
    best = fmin(train,
        space=hp.choice('gparams', [2,4,8,16,32,64]),
        algo=tpe.suggest,
        max_evals=MAX_EVALS,
        trials=trials)

    # log_string("\nBatch size with lowest loss = %s" % best)
    log_string("\nBatch size with the highest accuracy = %s" % best)

    with open(os.path.join(LOG_DIR, 'trials.txt'), 'w') as f:
        for item in trials.trials:
            f.write("\n%s\n" % item)

    log_string("\n--- Total running time for Hyperopt: %s ---\n" % (get_running_time(start_time_hype)))
...
def train(gparams):
    ...
    with tf.Graph().as_default():
        ...
        mean_loss = accuracy = avg_class_accuracy = 0  # reset the 3 para to 0

        for epoch in range(MAX_EPOCH):
            log_string('\n**** EPOCH %03d of %03d ****' % (epoch, MAX_EPOCH))
            log_string("Batch size = %02d" % (BATCH_SIZE))
            sys.stdout.flush()
            ...
            log_string('\n')
            log_string("--- Total running time for EPOCH %03d : %s ---" % (epoch, get_running_time(start_time_epoch)))
        ...
        'eval_time': time.time(),
        'Evaluate other params': {'mean loss': mean_loss, 'avg_class_accuracy': avg_class_accuracy},
        # -- attachments are handled differently
        'attachments':
        {'time_module': pickle.dumps(time.time)}
        }

        log_string("\n--- Total running time for Training: %s ---\n" % (get_running_time(start_time_train)))
